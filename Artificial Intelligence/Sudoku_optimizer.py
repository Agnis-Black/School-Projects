...
def cstAdd(problem, grid, domains,psize):
    ...
    for rowIdx in range(numRow):
      for colIdx in range(numCol):
        # starting from top leftmost element run constraint against consecutive row elements
        # grid[0][0] != grid[0][1] != grid[0][2] != grid[0][3] ...
        for colRunner in range(numCol):
          # Condition to prevent self equating constraint and redundancy
          if(colRunner>colIdx):
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx,colRunner]))
          
    # Adding constraints by columns
    for colIdx in range(numCol):
      for rowIdx in range(numRow):
        # starting from top leftmost element run constraint against consecutive columns elements
        # grid[0][0] != grid[1][0] != grid[2][0] != grid[3][0] ...
        for rowRunner in range(numRow):
          # Condition to prevent self equating constraint and redundancy
          if(rowRunner>rowIdx):
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowRunner,colIdx]))


    if(psize == 3):
      # Adding constraints by cubes
      for rowIdx in range(numRow):
        for colIdx in range(numCol):
          # starting from top leftmost element run constraint against consecutive row elements

          # Topmost left in 3x3 cube
          # grid[0][0] != grid[1][1] != grid[1][2] != grid[2][1] != grid[2][2] 
          if( (rowIdx % 3) == 0 ) and ( (colIdx % 3) == 0 ):
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+1,colIdx+1]))
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+2,colIdx+1]))
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+1,colIdx+2]))
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+2,colIdx+2]))

          # Topmost middle in 3x3 cube
          # grid[0][1] != grid[1][0] != grid[2][0] != grid[1][2] != grid[2][2] 
          if(( (rowIdx % 3) == 0 ) and ( (colIdx - 1) % 3 == 0 )):
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+1,colIdx-1]))
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+2,colIdx-1]))
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+1,colIdx+1]))
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+2,colIdx+1]))

          # Topmost right in 3x3 cube
          # grid[0][2] != grid[1][0] != grid[2][0] != grid[1][1] != grid[2][1] 
          if(( (rowIdx % 3) == 0 ) and ( (colIdx - 2) % 3 == 0 )):
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+1,colIdx-2]))
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+2,colIdx-2]))
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+1,colIdx-1]))
            problem.addConstraint(lambda a, b: a != b, (grid[rowIdx,colIdx], grid[rowIdx+2,colIdx-1]))
...
def fractionalKnapsack(c):
    ...
    # fixed weight capacity c
    # max item value in kanpsack while weight does not exceed capacity c
    # Fractions of items allowed
    # LP to solve the fractional knapsack problem with 3 items of weights 5, 3, and 1 and values
    # 2, 3, and 1 respectively for varying knapsack capacities.
    # only 1 unit of each item is available.


    x = np.array([1,1])
    y = np.array([3,3])
    z = np.array([5,2])
    items = np.array([x,y,z]) # weight, values
    #A = np.array([5,3,1])  # weights
    #b = np.array([2,3,1]) # values


    #f = cp.sum()
    # Put your constraints here
    # (x - A[i, :], 2) for i in range(m)]
    constraints = [  ]
