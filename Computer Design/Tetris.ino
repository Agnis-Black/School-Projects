...
// Aqsa Arif - 677417263
// Source code: https://www.hackster.io/archievkumar19/the-tetris-4de129
...
PROGMEM const short buttonRotate = A1; // Rotate in either direction
PROGMEM const short buttonRight = A0;  // translate right
PROGMEM const short buttonLeft = A0;   // translate left
PROGMEM const short buttonDown = A1;   // translate down
PROGMEM const short buttonPlay = A2;  // Play button
PROGMEM const short buttonPause = A2;  
PROGMEM const short buttonReset = A3;  // Reset button

const int buzzy = 8; //buzzer to arduino pin 8
...
{
  // move block left
  if (analogRead(buttonLeft) < 450 || readdata == "left")
  {
    //left
  ...
  // speed up fall
  if (analogRead(buttonDown) > 600 || readdata == "down")
  {
    //down
  ...
  // move block right
  if (analogRead(buttonRight) > 600 || readdata == "right")
  {
    //right
    bdelay = millis() + btsidedelay;
    return 3;
  }
  // rotate block by pushing analog up
  if (analogRead(buttonRotate) < 450)
  {
    //rotate
  ...
  // play buttonPlay
  if (analogRead(buttonPlay) > 800 || readdata == "play")
  {
    //rotate
    bdelay = millis() + buttondelay;
    return 5;
  ...
  // pause button
  if (analogRead(buttonPause) == 10000 || readdata == "pause")
  {
    //rotate
    bdelay = millis() + buttondelay;
    return 6;
  ...
  // reset button
  if (analogRead(buttonReset) > 800 || readdata == "reset")
  {
    //rotate
    bdelay = millis() + buttondelay;
    return 7;
...
// function to check for line space
...
void buzzer()
{
  for (int thisNote = 2; thisNote < 5; thisNote++)
  {
    ...
    tone(buzzy, melody[thisNote], noteDuration);
