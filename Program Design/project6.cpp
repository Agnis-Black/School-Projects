// Prog6Pentago.cpp
//     Play the two-player game of Pentago, trying to get 5 pieces in a row and undo a move.
//
// Author: Aqsa Arif
// Program: #6, Pentago using classes to undoing program 5
// 4/27/2020
//
...
// undo within a level. You should store the old board, the old score, the old playerToMove,
// and a pointer to the next Node.
class Node
{
public:
    Board theBoard;
    int moveCount;         // Stores the count
    char oldPieceToMove;   //Stores the previous piece
    Node* pNext;           //pointer to next move
};
...
void displayList( Node *pTemp)
{
    ...
    while(pTemp != nullptr) {
        cout << pTemp->moveCount << ":" << pTemp->oldPieceToMove;
...
//               restored from the list's next node
//    the playerToMove, which should be restored from the list's next node
//    the moveNumber, which should be restored from the list's next node

void deleteNodeFromList(Node* &pHead, Board &theBoard, char &playerToMove, int &moveCount )
{
    if (pHead->pNext == nullptr) {
        ...
        pHead = pHead->pNext;
        delete pTemp;
        // Restores to previous move
        theBoard = pHead->theBoard;
        playerToMove = pHead->oldPieceToMove;
        moveCount = pHead->moveCount;
    }
...
//    the playerToMove, which should be added to the list
//    the moveNumber, which should be added to the list
void addNodeToList(Node* &pHead, Board &theBoard, char &playerToMove, int &moveCount)
{
    // Create a new node and store current values into it
    // ...
    Node *pTemp = new Node;
    // storing new data into node
    pTemp->theBoard = theBoard;
    pTemp->oldPieceToMove = playerToMove;
    pTemp->moveCount = moveCount;
    // connect into list
    pTemp->pNext = pHead;
    pHead = pTemp;
...
int main() {
    ...
    addNodeToList(pHead, theBoard, pieceToMove, turnCount);
    ...
    while((userInput != 'x')&&(userInput != 'X')) {
        ...
        if(userInput == 'I'){
            ...
            deleteNodeFromList(pHead, theBoard, pieceToMove, turnCount);
            theBoard.displayBoard();
            displayList(pHead);
            cout << turnCount << ". Enter row, column, quadrant, direction for " << pieceToMove << ": " << endl;
            cin >> userInput;
